...
mod test {
    ...
    #[test]
    fn parse_ack_works() {
        let data = include_bytes!("test/message/acknowledge");
        let sgstate = new_connected_state();
        let packet = packet::Packet::read(data, &sgstate).unwrap();
    ...
    fn parse_local_join_works() {
        let data = include_bytes!("test/message/local_join");
        let sgstate = new_connected_state();
        let packet = packet::Packet::read(data, &sgstate).unwrap();
            ...
            packet::Packet::Message(header, message) => {
                ...
                match message {
                    Message::LocalJoin(data) => {
                        ...
                        assert_eq!(data.display_name, SGString::from_str(string::String::from("package.name.here")));
                    },
                    _ => panic!("Wrong type")
                }
            }
